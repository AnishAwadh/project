...
const Checkout = ({ cart }) => {
 ...
 function removethisitem(e) {
  const evey=e.currentTarget;
  evey.classList.add('hidden');
  cart.pop();
...
return (
  <div>
    <Script id={"paytm"} type="application/javascript" crossorigin="anonymous" src={`https://securegw-stage.paytm.in/theia/processTransaction/${process.env.NEXT_PUBLIC_MID}.js`} > </Script>
      ...
      <div className="container px-5 py-24 mx-auto min-h-screen">       
        <div className="flex flex-col w-full mb-12">
          ...
          <ul className='list-decimal px-8'>
            {cart.map((item) => {
              return <li className='currentlist flex item-center justiy-between space-x-40' key={Math.floor} >
                <h3>Product {item[0]} with a price of ₹{item[1]}</h3>
                <button className='py-1 px-3 rounded-full bg-black text-white' onClick={removethisitem}>X</button>
              </li>
            })}
          </ul>
