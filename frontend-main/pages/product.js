    ...
    <div className="flex flex-wrap -m-4">
      ...
      <div key={item.attributes.slug} className="xl:w-1/4 md:w-1/2 p-4">
        <div className="bg-gray-100 p-6 rounded-lg">
          <img className="h-72 rounded m-auto mb-8" src={`http://localhost:1337${item.attributes.image.data.attributes.url}`} alt="content"/>
...
export async function getServerSideProps(context) {
  ...
  let a = await fetch(process.env.STRAPI_URL + "/api/products?populate=*", {headers:headers})
